...
export default function IncomeCategoryDetailPage() {
  ...
  const fetchUsage = async () => {
    try {
      ...
      const data = await apiClient<UsageData>(`/income-categories/${categoryId}/usage?${queryParams}`);
      setData(data);
    } catch (error) {
        ...
        {loading ? (
          ...
          <Card className="w-full max-w-md">
            <CardContent className="py-8 text-center">
              ...
              <Button variant="outline" className="mt-4" onClick={() => router.push('/catalog/income-items')}>
                Quay lại danh sách
              </Button>
            </CardContent>
          </Card>
  ...
  return (
    <div>
      <PageHeader
        title={`Danh mục thu: ${category.name}`}
        description={`Mã: ${category.code}`}
        action={
          <Button variant="outline" onClick={() => router.push('/catalog/income-items')}>
        ...
        <CardContent className="pt-6">
          <div className="flex items-start gap-4">
            <VisualRenderer
              ...
              iconKey={category.iconKey || 'trending-up'}
            ...
            <div className="flex-1">
              ...
              <div className="flex flex-wrap gap-4">
                ...
                <div className="bg-green-50 px-4 py-2 rounded-lg">
                  <div className="text-sm text-green-600">Tổng tiền</div>
