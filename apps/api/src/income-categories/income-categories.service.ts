...
export class IncomeCategoriesService {
  ...
  private readonly CODE_PREFIX = 'DT';
  ...
  /**
   * Extract numeric part from code, handling malformed codes like 'DT0NaN'
   */
  ...
  async findOne(id: string) {
    ...
    if (!category) {
      throw new NotFoundException(`Danh mục thu không tồn tại`);
    }

    return category;
  }

  async create(data: Prisma.IncomeCategoryCreateInput, userId?: string, userEmail?: string) {
    ...
    while (retries < this.MAX_RETRIES) {
      try {
        result = await this.prisma.$transaction(async (tx) => {
          // 1. Get all DT-prefixed categories within transaction, sorted by code
          const categories = await tx.incomeCategory.findMany({
            where: {
              code: {
          ...
          this.logger.log(`[INCOME_CATEGORY_CREATE] Generated code: ${newCode} (highestNum: ${highestNum})`);

          // 4. Double-check code doesn't exist before creating
          const existing = await tx.incomeCategory.findUnique({
            where: { code: newCode },
          });

          if (existing) {
            this.logger.warn(`[INCOME_CATEGORY_CREATE] Code ${newCode} already exists in transaction!`);
        ...
        this.logger.log(`[INCOME_CATEGORY_CREATE] Created category with code ${result.code}`);
        break;
        ...
        if ((error instanceof Error && error.message === 'CODE_CONFLICT') ||
          ...
          retries++;
          this.logger.warn(
            `[INCOME_CATEGORY_CREATE] Duplicate code detected (attempt ${retries}/${this.MAX_RETRIES}), retrying...`
          );
          if (retries >= this.MAX_RETRIES) {
            this.logger.error('[INCOME_CATEGORY_CREATE] Max retries exceeded, giving up');
  ...
  async update(id: string, data: Prisma.IncomeCategoryUpdateInput, userId?: string, userEmail?: string) {
    ...
    const result = await this.prisma.incomeCategory.update({
      where: { id },
